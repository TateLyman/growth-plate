...
## CORR-266: the compound was misspecified for three rounds, and the objection I called decisive was probably never an objection

**Two things settled by going after one question the user asked.**

**(a) CD2665 is not the compound this proposition should be tested with, and its class cannot be determined.** Round 255 asked for an "RARγ antagonist"; round 257 corrected that to "inverse agonist" on mechanism; round 258 finds that the distinction has been formal chemistry since 1996 and that the compound in the only growth-plate experiment sits on the wrong side of the evidence.

`klein1996` built a C-1-substituted acetylenic retinoid series in which **every member antagonises RAR transactivation**, and showed the C-1 substituent splits them into **inverse agonists** and **neutral antagonists**. The discriminating assay is repression of **basal** activity in the absence of agonist — so *"selective RARβ/γ antagonist"*, which is all anyone says about CD2665, carries no information about class. `germain2009` then resolved it structurally: BMS204,493 is the inverse pan-RAR agonist, BMS195,614 the neutral RARα-selective antagonist, and helix H12 anchors differently in the two, which is what produces opposite coactivator/corepressor recruitment.

A Europe PMC full-text search pairing CD2665 with "inverse agonist", "neutral antagonist" or "corepressor" returns **zero records**. So `koyama2021` — the paper that opened this whole line — used a compound of unknown pharmacological class.

The retrospective consolation is real: **`riedl2020` already used the right compound.** Round 255 recorded it as "an RAR inverse agonist"; it is BMS204,493, *the* structurally characterised one. The human-cell arm was mechanistically aligned by accident.

**(b) The objection round 257 called the sharpest is probably not an objection.** I recorded `yamaguchi1998` — dominant-negative RAR in chondrogenic cells giving skeletal retardation **and cervical abnormalities** — as unexplained and standing against the ligand-less repressor model. `janesick2014` puts dominant-negative RARγ in the **same arm as the inverse agonist** — both *enhance* repression — and shows that arm raising **posterior Hox gene expression**. Raised posterior Hox expression is the classical cause of anterior-to-posterior homeotic transformation of the axial skeleton, which presents as **cervical abnormalities**. The phenotype is the predicted signature of enhanced repression acting on axial *patterning*, not evidence that enhanced repression shortens a growth plate — and it explains the one feature (cervical abnormalities) that a simple growth defect does not. Graded D: nobody measured Hox genes in those transgenics.
...
## CORR-269: I proposed a candidate the atlas had already closed, and the operator caught it

**What round 261 said, in its closing sentence.** That intermittent PTH1R agonism is *"the one with an unresolved term attribution and a decade of human paediatric safety data"* and is *"the obvious next target."*

**It was not unresolved. It was resolved against the candidate, in four independent ways, across rounds 184–198.**

1. **The final-length ledger is null.** FDA NDA 21-318 rat package, femoral length by QCT: **interim** at six months of treatment is positive and dose-dependent in both age windows (+1.5% at 5 µg/kg/d, +3.4% at 30, in the 2–8 month window; +1.5% and +2.9% in the 6–12 month window). **Terminal at 26 months is 0.0% in every arm**, including the two dosed continuously to the end. The two terminal positives in the literature use methods that cannot separate longitudinal from radial growth — `vahle2002` measured femoral **width** +32–33% in the same animals, and `jolette2006`'s BMA is length × width by construction, which CORR-189 covers by name. **The drug accelerates elongation onto an unchanged asymptote and the animal gives it back** — controls caught up 32.8 → 34.2 → 35.0 mm. It is a **rate agent, not a yield agent**, in the only species where final bone length was measured.
2. **The schedule is twenty times too frequent.** Rat proliferative chondrocyte cycle 1.4 d, human ~20 d. `ogawa2002`'s five-days-a-week rat regimen is **one pulse per cell cycle**; daily teriparatide in a human is **twenty**. The sign of this class inverts with schedule.
3. **The evidence base is one unreplicated abstract.** `ogawa2002` is the entire wild-type case, thirteen citations in twenty-four years, none replicating the growth finding, and `silvestrini2007` found growth plate width unaffected under PTH.
4. **The largest human dataset points the wrong way**, and the molecule flipped. `imaizumi2026`: 622 paediatric dialysis patients, each doubling of intact PTH associated with a fall in height velocity SDS of 0.26. And the TYMLOS label reports **26% neutralising antibodies** on abaloparatide with 2.4% cross-reacting with PTHrP — reproducing the PTHLH loss-of-function direction that causes brachydactyly type E **with short stature**, during a non-repeatable window.

The residue is narrow and the node carrying it is graded **D**: the rat gives the acceleration back because *the rat plate never closes*, and a human plate fuses — so whether acceleration is banked depends on whether fusion timing is set by hormonal milieu or by reserve exhaustion, which the rat data cannot distinguish. Plus a ~three-weekly teriparatide schedule nobody has tried.

**Rule, and it is the third instance in this session.** CORR-254 established: search the graph before proposing. I did not, twice more — once here, and once when I began re-deriving the PDE3/cilostazol argument from `kawabe2025` while `vosoritide_versus_cilostazol_decision` already held it, settled at round 80 on full texts of both PDE papers. **A ledger tool in `atlas/tools/` whose name matches the question is a mandatory read before the question is answered**, and `pth1r_final_length_ledger.py` had the answer printed in its own output.

### And the substantive half — the collision matrix

Applying this atlas's own design rule (*stack across control points, never within one*) to the current stack finds **three collisions**, one of them between the two agents this programme most wants to run together:

- **Vosoritide + erdafitinib are the same control point.** Both approved CNP-analogue labels place their mechanism at the **RAF-1 step of the FGFR3–MAPK cascade** — where erdafitinib already acts. Cilostazol's PKG → BK channel → TRPM7 → CaMKII chain is orthogonal to MAPK, which is why round 80 made cilostazol the better *partner* and vosoritide the better *monotherapy*.
- **Erdafitinib + cilostazol is a pharmacokinetic collision.** Erdafitinib is in vitro a time-dependent **inhibitor and inducer** of CYP3A4; cilostazol is a CYP3A4/2C19 substrate. Net exposure unpredictable. **The two collisions trade off against each other and neither pairing is clean.**
